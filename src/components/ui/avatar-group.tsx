      ...
      return (
        <>
          {avatarItems.slice(0, max).map((child, index) => {
            return cloneElement(child, {
              className: cn(
                ...
                'border-2 border-input bg-muted dark:border-slate-950',
              ),
              style: {
          ...
          {avatarItems.length > max && (
            <div
              className={cn(
                'relative flex items-center justify-center rounded-full border-2 border-input bg-muted dark:border-slate-950',
